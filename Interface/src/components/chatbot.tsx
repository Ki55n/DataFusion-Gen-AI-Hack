...
export default function Chatbot() {
  ...
  if (!isOpen) {
    return (
      <>
        {" "}
        <div className=" space-x-2  border-slate-800 rounded bg-slate-600 absolute inset-x-0 bottom-0 flex items-center justify-center">
          <button>
            <IoCubeOutline />
          </button>
          <button>
  ...
  return (
    <Modal isOpen={true} size={"full"} onClose={handleCloseModal}>
      <ModalContent>
        ...
        <div className={"flex flex-col  h-full "}>
          {chatHistory.length === 0 ? (
            <div className="text-center p-2 h-5/6">
              {/* Label */}

              {/* Paragraph */}
              <p>
                This is a financial AI chatbot built with Gemini API. You can
                start a conversation here or try the following questions:
                <br />
                - Most popular item
                <br />- Try data insights
              </p>
            </div>
          ) : (
            <div className="chat-history overflow-y-auto h-48 border rounded-lg p-2 h-5/6 p-28 relative">
              {chatHistory.map((message, index) => (
                <div key={index} className={`flex gap-2 `}>
                  <div
                    className={`flex w-full mb-5 gap-2 p-2 bg-primary text-white bg-slate-950 text-white`}
                  >
                    {message.user ? (
                      <CiUser className={"grow-0"} />
                    ) : (
                      <RiRobot3Fill className={"grow-0"} />
                    )}
                    <p className={"grow w-5/6"}>{message.message}</p>
                    <FaRegCopy className={"grow-0"} />
                    <Speaker
                      text={message.message}
                      child={<IoIosVolumeHigh className={"grow-0"} />}
                    />
                  </div>
                </div>
              ))}
              <div
                className={
                  "absolute bottom-0 right-0 left-0 flex justify-center p-3"
                }
              >
                <Button startContent={<FaRegCircleStop />} className={" w-1/6"}>
                  Stop Generating
                </Button>
              </div>
            </div>
